...
class SchemaError extends Error {
  constructor(problemGroups) {
    super("Schema errors encountered"), __publicField$1(this, "problemGroups"), this.problemGroups = problemGroups;
  }
  print(output) {
...
function getCrossDatasetReferenceMetadata(typeDef) {
  if (!isCrossDatasetReference(typeDef))
    return;
  ...
  function getDataset(type) {
    if (type) {
      if ("dataset" in type && typeof type.dataset == "string")
        return type.dataset;
      if (type.type)
        return getDataset(type.type);
    }
  }
...
class HelpfulError extends Error {
  constructor(message, helpUrl) {
    super(message), __publicField(this, "helpUrl"), this.helpUrl = helpUrl;
  }
}
