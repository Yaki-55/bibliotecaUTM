...
var debug$3 = require("debug"), node_fs = require("node:fs"), promises$1 = require("node:fs/promises"), node_os = require("node:os"), path = require("node:path"), promises = require("node:stream/promises"), fs = require("@sanity/util/fs"), asyncMutex = require("async-mutex"), isString = require("lodash/isString.js"), prettyMs = require("pretty-ms"), helpers = require("yargs/helpers"), yargs = require("yargs/yargs"), zlib = require("node:zlib"), rimraf = require("rimraf"), getIt = require("get-it"), middleware = require("get-it/middleware"), node_stream = require("node:stream"), consoleTablePrinter = require("console-table-printer"), dateFns = require("date-fns"), fs$1 = require("fs"), path$1 = require("path"), logSymbols = require("log-symbols"), oneline = require("oneline"), url = require("url"), EventSource = require("@sanity/eventsource"), rxjs = require("rxjs"), exportDataset = require("@sanity/export"), fs$2 = require("fs/promises"), sanityImport = require("@sanity/import"), padStart = require("lodash/padStart.js"), uuid = require("@sanity/uuid"), chokidar = require("chokidar"), execa = require("execa"), json5 = require("json5"), isEqual = require("lodash/isEqual.js"), isPlainObject = require("lodash/isPlainObject.js"), noop$1 = require("lodash/noop.js"), os = require("os"), pluralize = require("pluralize-esm"), tokenize = require("json-lexer"), open = require("open"), groupBy = require("lodash/groupBy.js"), util = require("util"), deburr = require("lodash/deburr.js"), node = require("esbuild-register/dist/node"), migrate = require("@sanity/migrate"), node_tty = require("node:tty"), sanity = require("sanity"), size = require("lodash/size.js"), sortBy = require("lodash/sortBy.js");
function _interopDefaultCompat(e) {
  return e && typeof e == "object" && "default" in e ? e : { default: e };
}
var debug__default = /* @__PURE__ */ _interopDefaultCompat(debug$3), path__default = /* @__PURE__ */ _interopDefaultCompat(path), isString__default = /* @__PURE__ */ _interopDefaultCompat(isString), prettyMs__default = /* @__PURE__ */ _interopDefaultCompat(prettyMs), yargs__default = /* @__PURE__ */ _interopDefaultCompat(yargs), zlib__default = /* @__PURE__ */ _interopDefaultCompat(zlib), rimraf__default = /* @__PURE__ */ _interopDefaultCompat(rimraf), fs__default = /* @__PURE__ */ _interopDefaultCompat(fs$1), path__default$1 = /* @__PURE__ */ _interopDefaultCompat(path$1), logSymbols__default = /* @__PURE__ */ _interopDefaultCompat(logSymbols), oneline__default = /* @__PURE__ */ _interopDefaultCompat(oneline), url__default = /* @__PURE__ */ _interopDefaultCompat(url), EventSource__default = /* @__PURE__ */ _interopDefaultCompat(EventSource), exportDataset__default = /* @__PURE__ */ _interopDefaultCompat(exportDataset), fs__default$1 = /* @__PURE__ */ _interopDefaultCompat(fs$2), sanityImport__default = /* @__PURE__ */ _interopDefaultCompat(sanityImport), padStart__default = /* @__PURE__ */ _interopDefaultCompat(padStart), chokidar__default = /* @__PURE__ */ _interopDefaultCompat(chokidar), execa__default = /* @__PURE__ */ _interopDefaultCompat(execa), json5__default = /* @__PURE__ */ _interopDefaultCompat(json5), isEqual__default = /* @__PURE__ */ _interopDefaultCompat(isEqual), isPlainObject__default = /* @__PURE__ */ _interopDefaultCompat(isPlainObject), noop__default = /* @__PURE__ */ _interopDefaultCompat(noop$1), os__default = /* @__PURE__ */ _interopDefaultCompat(os), pluralize__default = /* @__PURE__ */ _interopDefaultCompat(pluralize), tokenize__default = /* @__PURE__ */ _interopDefaultCompat(tokenize), open__default = /* @__PURE__ */ _interopDefaultCompat(open), groupBy__default = /* @__PURE__ */ _interopDefaultCompat(groupBy), deburr__default = /* @__PURE__ */ _interopDefaultCompat(deburr), size__default = /* @__PURE__ */ _interopDefaultCompat(size), sortBy__default = /* @__PURE__ */ _interopDefaultCompat(sortBy);
...
async function downloadAsset(url2, fileName, fileType, outDir) {
  ...
  await withRetry(async () => {
    ...
    debug$1("Received asset %s with status code %d", normalizedFileName, response == null ? void 0 : response.statusCode), await promises.pipeline(response.body, node_fs.createWriteStream(assetFilePath));
  });
}
...
const downloadBackupCommand = {
  ...
  action: async (args, context) => {
    ...
    const start = Date.now(), progressSpinner = newProgress(output, "Setting up backup environment..."), tmpOutDir = await promises$1.mkdtemp(path__default.default.join(node_os.tmpdir(), "sanity-backup-"));
    for (const dir of [outDir, path__default.default.join(tmpOutDir, "images"), path__default.default.join(tmpOutDir, "files")])
      node_fs.mkdirSync(dir, { recursive: !0 });
    debug("Writing to temporary directory %s", tmpOutDir);
    const tmpOutDocumentsFile = path__default.default.join(tmpOutDir, "data.ndjson"), docOutStream = node_fs.createWriteStream(tmpOutDocumentsFile), docWriteMutex = new asyncMutex.Mutex();
    try {
...
async function prepareBackupOptions(context, args) {
  ...
  let out = await (async () => flags.out !== void 0 ? fs.absolutify(flags.out) : await prompt.single({
    type: "input",
    message: "Output path:",
    default: path__default.default.join(workDir, defaultOutFileName),
    filter: fs.absolutify
  }))();
  return isPathDirName(out) && (out = path__default.default.join(out, defaultOutFileName)), !flags.overwrite && node_fs.existsSync(out) && (await prompt.single({
    type: "confirm",
...
`, addCorsOriginCommand = {
  ...
  action: async (args, context) => {
    ...
    fs__default.default.existsSync(path__default$1.default.join(process.cwd(), origin)) && output.warn(`Origin "${origin}?" Remember to quote values (sanity cors add "*")`), await addCorsOrigin(origin, flags, context) && output.print("CORS origin added successfully");
  }
}, corsGroup = {
  name: "cors",
  signature: "[COMMAND]",
...
const createAliasHandler = async (args, context) => {
  ...
  if (aliasName.startsWith(ALIAS_PREFIX) ? aliasName = aliasName.substring(1) : aliasOutputName = `${ALIAS_PREFIX}${aliasName}`, aliases.includes(aliasName))
...
const unlinkAliasHandler = async (args, context) => {
  ...
  let aliasName = await (alias || promptForDatasetAliasName(prompt)), aliasOutputName = aliasName;
  aliasName.startsWith(ALIAS_PREFIX) ? aliasName = aliasName.substring(1) : aliasOutputName = `${ALIAS_PREFIX}${aliasName}`;
  const linkedAlias = fetchedAliases.find((elem) => elem.name === aliasName);
  if (!linkedAlias)
    throw new Error(`Dataset alias "${aliasOutputName}" does not exist`);
...
const exportDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    destinationPath || (destinationPath = await prompt.single({
      type: "input",
      message: "Output path:",
      default: path__default$1.default.join(workDir, `${dataset}.tar.gz`),
      filter: fs.absolutify
    }));
...
async function getOutputPath(destination, dataset, prompt, flags) {
  ...
  const finalPath = looksLikeFile ? dstPath : path__default$1.default.join(dstPath, `${dataset}.tar.gz`);
...
const importDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    if (isUrl)
      ...
      const sourceFile = path__default$1.default.resolve(process.cwd(), file), fileStats = await fs__default$1.default.stat(sourceFile).catch(() => null);
      if (!fileStats)
        throw new Error(`${sourceFile} does not exist or is not readable`);
      sourceIsFolder = fileStats.isDirectory(), sourceIsFolder ? inputStream = sourceFile : (assetsBase = path__default$1.default.dirname(sourceFile), inputStream = await fs$1.createReadStream(sourceFile));
    }
    ...
    function endTask({ success }) {
      if (spinInterval && clearInterval(spinInterval), spinInterval = null, success && stepStart && currentProgress) {
        ...
        currentProgress && currentProgress.fail();
    }
    try {
...
`, createDocumentsCommand = {
  ...
  action: async (args, context) => {
    ...
    if ((replace || missing) && (operation = replace ? "createOrReplace" : "createIfNotExists"), file) {
      const contentPath = path__default$1.default.resolve(process.cwd(), file), content = json5__default.default.parse(await fs__default$1.default.readFile(contentPath, "utf8")), result = await writeDocuments(content, operation, client);
      output.print(getResultMessage(result, operation));
      return;
    }
    const docId = id || uuid.uuid(), ext = useJson5 ? "json5" : "json", tmpFile = path__default$1.default.join(os__default.default.tmpdir(), "sanity-cli", `${docId}.${ext}`), stringify = useJson5 ? json5__default.default.stringify : JSON.stringify, defaultValue = id && await client.getDocument(id) || { _id: docId, _type: "specify-me" };
    await fs__default$1.default.mkdir(path__default$1.default.join(os__default.default.tmpdir(), "sanity-cli"), { recursive: !0 }), await fs__default$1.default.writeFile(tmpFile, stringify(defaultValue, null, 2), "utf8");
...
function printMessage(message, context, options) {
  ...
  output.print(`Date: ${message.createdAt}`), output.print(`Status: ${message.status}`), output.print(`Result code: ${message.resultCode}`), message.failureCount > 0 && output.print(`Failures: ${message.failureCount}`), detailed && (output.print("Payload:"), output.print(util.inspect(JSON.parse(message.payload), { colors: !0 }))), detailed && message.attempts && (output.print("Attempts:"), message.attempts.forEach((attempt) => {
...
`, TEMPLATES = [
  ...
  action: async (args, context) => {
    ...
    if (node_fs.existsSync(destDir) && !await prompt.single({
      type: "confirm",
      message: `Migration directory ${chalk.cyan(destDir)} already exists. Overwrite?`,
      default: !1
    }))
      return;
    node_fs.mkdirSync(destDir, { recursive: !0 });
    ...
    await promises$1.writeFile(definitionFile, renderedTemplate), output.print(), output.print(`${chalk.green("\u2713")} Migration created!`), output.print(), output.print("Next steps:"), output.print(
      `Open ${chalk.bold(
        definitionFile
...
async function resolveMigrations(workDir) {
  ...
  const migrationsDir = path__default.default.join(workDir, MIGRATIONS_DIRECTORY), migrationEntries = await promises$1.readdir(migrationsDir, { withFileTypes: !0 }), migrations = [];
...
function convertToTree(nodes) {
  ...
  function addNode(node2, tree = root) {
    ...
    const [current, ...rest] = node2.path, key = sanity.pathToString([current]);
...
const runMigrationCommand = {
 ...
 - `)}`
    ...
    if ((flags.confirm && await prompt.single({
