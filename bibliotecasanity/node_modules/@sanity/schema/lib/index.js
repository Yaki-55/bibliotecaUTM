...
var omit = require("lodash/omit.js"), pick = require("lodash/pick.js"), arrify = require("arrify"), isUndefined = require("lodash/isUndefined.js"), omitBy = require("lodash/omitBy.js"), capitalize = require("lodash/capitalize.js"), resolve = require("./_chunks-cjs/resolve.js"), castArray = require("lodash/castArray.js"), flatMap = require("lodash/flatMap.js"), startCase = require("lodash/startCase.js"), isPlainObject = require("lodash/isPlainObject.js"), toPath = require("lodash/toPath.js");
...
const OVERRIDABLE_FIELDS$d = [...DEFAULT_OVERRIDEABLE_FIELDS], ARRAY_CORE = {
  ...
  extend(subTypeDef, createMemberType) {
    const parsed = Object.assign(pick__default.default(ARRAY_CORE, OVERRIDABLE_FIELDS$d), subTypeDef, {
      type: ARRAY_CORE
    });
...
const DEFAULT_LINK_ANNOTATION = {
  ...
  extend(subTypeDef, extendMember) {
    const options = { ...subTypeDef.options || DEFAULT_OPTIONS$3 }, { marks, styles, lists, of, ...rest } = subTypeDef, childrenField = createChildrenField(marks, of), styleField = createStyleField(styles), listItemField = createListItemField(lists), markDefsField = {
      ...
      subTypeDef.fields || []
    ), parsed = Object.assign(pick__default.default(BLOCK_CORE, INHERITED_FIELDS$1), rest, {
      type: BLOCK_CORE,
      options
    });
...
const INHERITED_FIELDS = [
  ...
  extend(subTypeDef, extendMember) {
    const options = { ...subTypeDef.options || DEFAULT_OPTIONS$2 }, { annotations = [], marks = [] } = subTypeDef, fields = [MARKS_FIELD, TEXT_FIELD], parsed = Object.assign(pick__default.default(SPAN_CORE, INHERITED_FIELDS), subTypeDef, {
      type: SPAN_CORE,
      options
    });
    return lazyGetter(parsed, "fields", () => fields.map((fieldDef) => {
...
const OVERRIDABLE_FIELDS$c = [...DEFAULT_OVERRIDEABLE_FIELDS], BOOLEAN_CORE = {
  ...
  extend(subTypeDef) {
    const parsed = Object.assign(pick__default.default(BOOLEAN_CORE, OVERRIDABLE_FIELDS$c), subTypeDef, {
      type: BOOLEAN_CORE,
...
const CrossDatasetReferenceType = {
  ...
  extend(subTypeDef, createMemberType) {
    ...
    const parsed = Object.assign(
      pick__default.default(CROSS_DATASET_REFERENCE_CORE, OVERRIDABLE_FIELDS$b),
      subTypeDef,
      {
  ...
  extend(subTypeDef) {
    const parsed = Object.assign(pick__default.default(DATETIME_CORE, OVERRIDABLE_FIELDS$9), subTypeDef, {
      type: DATETIME_CORE,
      preview: primitivePreview
    });
    return subtype(parsed);
...
const OVERRIDABLE_FIELDS$8 = [
  ...
  extend(rawSubTypeDef, createMemberType) {
    const subTypeDef = { fields: [], ...rawSubTypeDef }, options = { ...subTypeDef.options || {} }, parsed = Object.assign(pick__default.default(this.get(), OVERRIDABLE_FIELDS$8), subTypeDef, {
      type: this.get(),
...
const DOCUMENT_CORE = {
  ...
  extend(rawSubTypeDef, createMemberType) {
    const options = { ...rawSubTypeDef.options || DEFAULT_OPTIONS$1 }, fields = [ASSET_FIELD$1, ...rawSubTypeDef.fields || []], subTypeDef = { ...rawSubTypeDef, fields }, parsed = Object.assign(pick__default.default(FILE_CORE, OVERRIDABLE_FIELDS$6), subTypeDef, {
      type: FILE_CORE,
      title: subTypeDef.title || (subTypeDef.name ? startCase__default.default(subTypeDef.name) : ""),
      options,
      fields: subTypeDef.fields.map((fieldDef) => {
  ...
  extend(subTypeDef) {
    const parsed = Object.assign(pick__default.default(NUMBER_CORE, OVERRIDABLE_FIELDS$4), subTypeDef, {
      type: NUMBER_CORE,
      preview: primitivePreview
    });
    return subtype(parsed);
...
const ReferenceType = {
  ...
  extend(subTypeDef, createMemberType) {
    ...
    })), lazyGetter(parsed, "fieldsets", () => createFieldsets(subTypeDef, parsed.fields)), lazyGetter(parsed, "to", () => arrify__default.default(subTypeDef.to).map((toType) => createMemberType(toType))), lazyGetter(parsed, "title", () => subTypeDef.title || buildTitle(parsed)), subtype(parsed);
    function subtype(parent) {
      return {
        get() {
          return parent;
  ...
  extend(subTypeDef) {
    const parsed = Object.assign(pick__default.default(STRING_CORE, OVERRIDABLE_FIELDS$2), subTypeDef, {
      type: STRING_CORE,
      preview: primitivePreview
    });
    return subtype(parsed);
  ...
  extend(subTypeDef) {
    const parsed = Object.assign(pick__default.default(TEXT_CORE, OVERRIDABLE_FIELDS$1), subTypeDef, {
      type: TEXT_CORE,
      preview: primitivePreview
    });
    return subtype(parsed);
  ...
  extend(subTypeDef) {
    const parsed = Object.assign(pick__default.default(URL_CORE, OVERRIDABLE_FIELDS), subTypeDef, {
      type: URL_CORE,
      preview: primitivePreview
    });
    return subtype(parsed);
...
var types = /* @__PURE__ */ Object.freeze({
  ...
  text: TextType,
  url: UrlType
}), __defProp = Object.defineProperty, __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: !0, configurable: !0, writable: !0, value }) : obj[key] = value, __publicField = (obj, key, value) => (__defNormalProp(obj, typeof key != "symbol" ? key + "" : key, value), value);
