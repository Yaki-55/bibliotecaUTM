...
class BufferList {
  constructor() {
    __publicField(this, "buffered"), __publicField(this, "shifted"), __publicField(this, "queue"), __publicField(this, "_offset"), this.buffered = 0, this.shifted = 0, this.queue = new FIFO__default.default(), this._offset = 0;
  }
  push(buffer) {
...
function decodeOct(val, offset, length) {
  if (val = val.subarray(offset, offset + length), offset = 0, val[offset] & 128)
    return parse256(val);
  for (; offset < val.length && val[offset] === 32; )
    offset++;
  const end = clamp(indexOf(val, 32, offset, val.length), val.length, val.length);
  for (; offset < end && val[offset] === 0; )
    offset++;
...
async function assert2xx(res) {
  if (res.status < 200 || res.status > 299) {
    ...
    }), message = response.error ? response.error.description : `HTTP Error ${res.status}: ${res.statusText}`, err = new Error(message);
    throw err.statusCode = res.status, err;
  }
}
async function fetchStream({ url, init }) {
  const response = await fetch(url, init);
  if (await assert2xx(response), response.body === null)
...
function createFilteredDocumentsClient(getFilteredDocumentsReadableStream) {
  ...
  async function getDocumentsFromBuffer(ids) {
    ...
    for await (const doc of getAllDocumentsFromBuffer())
      if (ids.includes(doc._id) && (remaining--, found[doc._id] = doc), remaining === 0)
        break;
